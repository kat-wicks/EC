...
def evaluate_solution(solution, best_defender, X, y):
    """
    ...
    masked_X = X[solution]
    masked_y= y[solution]
    pca_scores = best_defender[0].fit_transform(masked_X)
    knn = best_defender[1].fit(pca_scores,masked_y)
    score = best_defender[1].score(pca_scores,masked_y)
    return -1*score
